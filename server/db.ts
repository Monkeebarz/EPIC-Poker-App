...
import {
  ...
  savedStructures,
  InsertSubscriptionTier, subscriptionTiers,
} from "../drizzle/schema";
...
// ============ SUBSCRIPTION TIER QUERIES (preserved from current main) ============

export async function listSubscriptionTiers() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(subscriptionTiers).orderBy(asc(subscriptionTiers.sortOrder), asc(subscriptionTiers.id));
}

export async function getSubscriptionTierById(id: number) {
  const db = await getDb();
  if (!db) return undefined;
...
export async function createSubscriptionTier(tier: InsertSubscriptionTier) {
  ...
  return getSubscriptionTierById(id);
}

export async function updateSubscriptionTier(id: number, tier: Partial<InsertSubscriptionTier>) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.update(subscriptionTiers).set(tier).where(eq(subscriptionTiers.id, id));
  return getSubscriptionTierById(id);
}

export async function deleteSubscriptionTier(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
