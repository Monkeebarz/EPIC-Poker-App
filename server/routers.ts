...
function requireAdminSession(req: Parameters<typeof hasAdminSession>[0]) {
  if (!hasAdminSession(req)) throw new TRPCError({ code: "UNAUTHORIZED", message: "Admin session required" });
}
...
export const appRouter = router({
  ...
  admin: router({
    login: publicProcedure.input(z.object({ password: z.string() })).mutation(({ input, ctx }) => {
      if (!isAdminPasswordValid(input.password)) throw new TRPCError({ code: "UNAUTHORIZED", message: "Incorrect admin password" });
    ...
    updateTier: publicProcedure.input(z.object({ id: z.number().int().positive(), data: tierInput })).mutation(({ input, ctx }) => {
      requireAdminSession(ctx.req);
      return updateSubscriptionTier(input.id, { ...input.data, description: input.data.description || null, perks: input.data.perks });
    }),
    ...
    startGame: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const room = getOrCreateRoom(
          ...
          parseInt(tournament.tableSize, 10)
        );
